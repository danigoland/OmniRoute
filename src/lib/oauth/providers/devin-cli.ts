...
import { mapDevinSessionToken } from "./windsurf-oauth";

/**
 * Devin CLI OAuth Provider — import-token only.
 *
 * Distinct from the `windsurf` provider despite sharing this config. `devin-cli`
 * is served by `DevinCliExecutor`, which drives the locally installed Devin CLI
 * binary over ACP; it never speaks the Cascade Connect protocol. A Devin session
 * JWT minted by the browser PKCE flow is therefore useless here — the ACP
 * transport rejects it with `-32602 Invalid params` — so browser login stays
 * off for this entry and only `windsurf` exposes it.
 *
...
export const devinCli = {
   ...
   * the token happens to be a JWT) matches the `windsurf` entry.
   */
