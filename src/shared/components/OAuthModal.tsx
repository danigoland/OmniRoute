 ...
 */
const PKCE_PASTE_RECOVERABLE_PROVIDERS = new Set(["devin-desktop"]);
...
// devin-desktop is absent: it offers BOTH browser PKCE login and API-key paste,
// so it needs the tab bar that this set suppresses.
const IMPORT_TOKEN_ONLY_PROVIDERS = new Set(["devin-cli"]);
...
export default function OAuthModal({
  ...
  const handleSaveToken = useCallback(async () => {
    ...
    setError(null);
    try {
      // The login helper prints a credential blob rather than a bare key; that
      // has to go to the blob endpoint, not /import-token.
      if (isCredentialBlob(raw)) {
        await submitCredentialBlob(provider, raw, reauthConnection, setStep, onSuccess);
        return;
      }
  ...
  const startOAuthFlow = useCallback(
    async (opts?: { grokBrowser?: boolean }) => {
      ...
      try {
        ...
        } else if (provider === "devin-desktop") {
